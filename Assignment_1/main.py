...
def task1():
    ...
    """ Start of your code
    """
    py = lambda y: np.exp(-0.5*y)/(np.sqrt(2*np.pi)*np.sqrt(y))

    # Plot the transformed pdf
    x = np.linspace(0.01, 5, 100)  # Avoid zero for plotting
    plt.plot(x, py(x))
    plt.xlabel('y')
    plt.ylabel('p$_Y$(y)')
    plt.title('Transformed PDF p$_Y$(y)')
    plt.grid()
    plt.xlim([-2, 5])
    #plt.show()
    plt.savefig('transformed_pdf_plot.png')

    # Integrate the pdf over different ranges
    inf_inf = quad(py, -np.inf, np.inf)
    zero_inf = quad(py, 0, np.inf)
    inf_zero = quad(py, -np.inf, 0)

    # Print the integration results
    print('from -inf to inf:', inf_inf)
    print('from 0 to inf:', zero_inf)
    print('from -inf to 0:', inf_zero)
